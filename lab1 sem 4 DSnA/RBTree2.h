...
public:
	Elem* root; Elem* nill;
	/*class TreeIterator: public Iterator<Elem*>
	{public:
		friend class RBtree;
		Stack<Elem*>* stack;
		Elem* cur;
...
/*	~RBTree()
	...
	List<Key>* getKeys()
	{
		...
		RBTreeIterator* iterator = createIterator();
		//while(iterator->has_next())
		while (iterator->has_next())
		{
			Pair<Key,Value>  next = iterator->next();
			keys->pushBack(next.first);
		}
		delete iterator;
		return keys;
	}
